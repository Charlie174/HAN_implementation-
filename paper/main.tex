...
Test-set composition sensitivity confirms $\text{F1-Macro} = 0.9397 \pm 0.0013$.
...
\textbf{Interpretable and pre-trained EHR models.}
RETAIN \cite{choi_2016_retain} introduces reverse-time attention for
interpretable single-visit prediction; we include it as a baseline
(F1-Macro = 0.6111 on our dataset --- it fails on rare diseases with
$< 5\%$ prevalence).
BEHRT \cite{li_behrt_2020} applies BERT-style pre-training to sequences
of ICD diagnostic codes, and GraphCare \cite{jiang_graphcare_2024}
constructs personalized knowledge graphs from medical ontologies.
These models target different data modalities (diagnostic code sequences,
ontology-linked visits) and require pre-training corpora or external
knowledge bases not available in our setting, which uses raw laboratory
test values as input features.
Direct empirical comparison is deferred to future work; HAN++ addresses a
complementary niche --- heterogeneous meta-path reasoning over structured
laboratory values with patient-conditioned interpretability.
...
\begin{table*}[t]
  ...
  \begin{tabular}{llccccc}
    ...
    RETAIN \cite{choi_2016_retain} & DL (Interp. Baseline) & --- & 0.9383 & 0.6111 & --- & --- \\
    \midrule
    ...
    \multicolumn{7}{l}{\small \phantom{$^{\ddagger}$}capabilities absent from all traditional baselines.
      Test-set sensitivity: $0.9397\pm0.0013$ (Section~\ref{subsec:multiseed}).} \\
  \end{tabular}
\end{table*}

Fig.~\ref{fig:comparison} visualises F1-Macro, Recall-Macro, and rare-disease
F1 across all model families.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig4_model_comparison.pdf}
  \caption{\textbf{(a) Overall performance.}
           F1-Macro (solid) and Recall-Macro (hatched) for all models on the
           same 9\,718-patient test split (CareAI April 2026, 9 diseases).
           Traditional tree-based classifiers (DT, RF) achieve
           F1-Macro\,$\approx$\,0.946 with GridSearch tuning.
           Vanilla HAN (global query) drops to 0.857 despite graph structure,
           revealing that a shared meta-path weight is insufficient.
           HAN++ (patient-conditioned) recovers to 0.938 with
           AUC-ROC\,=\,0.999 vs.\ DT 0.985.
           Naive Bayes achieves high Recall (0.984) but negligible
           precision (0.25), yielding F1\,=\,0.322.
           KNN Recall-Macro\,=\,0.768 indicates 23\% of disease cases missed.
           \textbf{(b) Rare-disease F1.}
           For the three rarest diseases ($\leq$3.3\% prevalence),
           where clinical impact is highest,
           HAN++ outperforms all baselines by large margins —
           most notably Dyslipidemia (1.1\%):
           KNN\,=\,0.745, DT\,=\,0.842, HAN\,=\,0.714, HAN++\,=\,\textbf{0.967}
           (+25\,pp over vanilla HAN, +13\,pp over best traditional).}
...
\textbf{GNN/DL architecture progression.}
Within neural approaches, the improvement
SeHGNN (0.63) $\to$ RETAIN (0.61) $\to$ pyHGT (0.76) $\to$
HGT-HAN (0.84) $\to$ HAN++ (0.94)
confirms that hierarchical meta-path attention with patient conditioning
is the key architectural driver.
Notably, RETAIN --- the standard interpretable deep baseline --- achieves
F1 = 0 on three rare diseases ($< 5\%$ prevalence), whereas HAN++'s
graph-relational structure handles class imbalance substantially better.
...
\subsection{Test-Set Composition Sensitivity}
\label{subsec:multiseed}

To assess whether reported metrics depend on a favourable random test split,
we vary the data-split seed across three values (42, 123, 456) while holding
model weights fixed (trained once with seed = 42).
F1-Macro = $0.9397 \pm 0.0013$, F1-Micro = $0.9534 \pm 0.0004$,
AUC-ROC = $0.9971 \pm 0.0001$ (Fig.~\ref{fig:per_disease}).
The negligible standard deviation ($\leq 0.0013$) across splits confirms that
the reported result is robust to test-set composition and does not represent
a statistical outlier.
We note that this experiment holds model weights constant and varies only the
evaluation split; full training stability (retraining with different weight
initialisations) is deferred to future work.
...
Among the 4.3\% of patients who are P-O-P-dominant
($\beta_{\text{P-O-P}} > 0.5$), 81.8\% show logit change $> 0.1$ when
P-O-P is disabled, confirming attention faithfulness for this subgroup.
For the majority P-D-P-dominant group, 43.8\% are sensitive to P-D-P
removal --- lower than expected, likely because P-D-P carries the primary
information signal and the dual-path architecture provides partial
compensation when one path is disabled.
This asymmetry is consistent with a model that relies heavily on P-D-P
but where P-O-P-dominant patients genuinely depend on organ-based
reasoning.
We note that attention weights do not constitute complete causal
explanations \cite{jain_2019}; they are a necessary but not sufficient
condition for interpretability, complemented here by the counterfactual
masking evidence above.
...
($\rho \in \{0.50, 0.70, 0.90, 0.99\}$, where higher $\rho$ retains more symptoms)
...
filter is never triggered at any threshold below $\rho = 0.99$.
This null result means the threshold ablation is uninformative on this
particular dataset; on datasets with high-frequency hub symptoms
(e.g., CBC, glucose appearing in $> 50\%$ of patients), this parameter
would be load-bearing.
We report the null result for transparency: HAN++ performance is not an
artefact of a particular pruning setting, but neither can we claim
robustness to meaningful graph sparsity variation on this data.
...
Within the GNN/DL family, HAN++ shows clear improvement over all
alternatives: SeHGNN (0.63) $\to$ RETAIN (0.61) $\to$ pyHGT (0.76) $\to$
HGT-HAN (0.84) $\to$ HAN++ (0.94).
...
\textbf{Limitations.}
\emph{(1)~Rule-based label bias.}
The high-confidence dataset used for evaluation derives disease labels
from clinical reference-range thresholds, which inherently advantages
threshold-learning classifiers (DT, RF).
Evaluation on physician-assigned diagnoses would better isolate
relational reasoning benefits.
\emph{(2)~Single-centre data.}
All data originates from CareCode (PVT) Ltd hospitals in southern
Sri Lanka; cross-geography and cross-institution validation is
required before wider deployment.
\emph{(3)~No temporal modelling.}
HAN++ processes a single aggregate snapshot per patient; multi-visit
temporal modelling (e.g., GRU over visit sequences \cite{li_2021})
is not incorporated.
\emph{(4)~Comparator scope.}
While we include RETAIN as an interpretable deep baseline, we do not
compare against recent transformer-based EHR models
(BEHRT \cite{li_behrt_2020}, GraphCare \cite{jiang_graphcare_2024})
as these target different data modalities (diagnostic codes, temporal
visit sequences) and require pre-training corpora unavailable in our
setting; direct comparison would strengthen positioning.
\emph{(5)~Patient-conditioned attention variance.}
Mean $\bar{\beta}_{\text{P-D-P}} = 0.923$ indicates limited per-patient
variation in semantic weights on this dataset; datasets with more
heterogeneous patient profiles may benefit more from patient conditioning.
\emph{(6)~Training stability.}
The multi-seed experiment (Section~\ref{subsec:multiseed}) varies test
splits with fixed weights; full retraining stability across random
initialisations remains future work.
This study follows the TRIPOD reporting guidelines for prediction model
transparency \cite{collins_2015}.

\textbf{Future Work.}
(i)~Incorporating unstructured clinical notes via Graph RAG \cite{peng_2024};
(ii)~temporal multi-visit modelling;
(iii)~expanding external validation cohorts \cite{riley_2021};
(iv)~hub-aware graph pruning (HSGNN \cite{liu_2020});
and (v)~prospective calibration validation for regulatory-grade
